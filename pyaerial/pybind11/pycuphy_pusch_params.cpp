...
void printPuschStatPrms(const cuphyPuschStatPrms_t& statPrms) {
    ...
    printf("enableUlRxBf:               %4d\n", statPrms.enableUlRxBf);
    printf("ldpcnIterations:            %4d\n", statPrms.ldpcnIterations);
...
size_t PuschParams::getBufferSize() const {
    ...
    nBytesBuffer += maxNumRmLLRsPerCb * MAX_N_CBS_PER_TB_PER_CELL_GROUP_SUPPORTED + LINEAR_ALLOC_PAD_BYTES;
...
void PuschParams::setStatPrms(const py::object& statPrms) {
    ...
    m_puschStatPrms.enableUlRxBf              = statPrms.attr("enableUlRxBf").cast<uint8_t>();
    m_puschStatPrms.ldpcnIterations           = statPrms.attr("ldpcnIterations").cast<uint8_t>();
    ...
    m_puschStatPrms.nMaxLdpcHetConfigs        = 32;
    ...
    m_puschStatPrms.enableCsiP2Fapiv3         = 0;  // Disabled.
    ...
    //m_puschStatPrms.workCancelMode  = cuphyPuschWorkCancelMode_t::NO_WORK_CANCEL; // could overwrite value for local testing
...
void PuschParams::setDynPrmsPhase1(const py::object& dynPrms) {
    ...
    for (int cellIdx = 0; cellIdx < nCells; cellIdx++) {
        py::array tDataRx = tDataRxList[cellIdx];

        // Get dimensions and size to be allocated.
        py::buffer_info buf = tDataRx.request();
        size_t rxDataBytes = N_BYTES_C16 * buf.shape[0] * buf.shape[1] * buf.shape[2];

        m_tDataRx[cellIdx] = deviceFromNumpy<std::complex<float>, py::array::f_style | py::array::forcecast>(
            tDataRx,
            m_LinearAlloc.alloc(rxDataBytes),
            CUPHY_C_32F,
            CUPHY_C_16F,
...
void PuschParams::setDynPrmsPhase2(const py::object& dynPrms) {
    ...
    for(int k = 0; k < m_puschDynPrms.pDataOut->totNumTbs; k++) {
        try {
            ...
            CUDA_CHECK(cudaStreamSynchronize(m_puschDynPrms.phase1Stream));
...
uint32_t PuschParams::updatePuschRxUeGrpPrms(cudaStream_t cuStream) {
    ...
    uint32_t nMaxPrb = PuschRx::expandFrontEndParameters(&m_puschDynPrms, &m_puschStatPrms, m_drvdUeGrpPrmsCpu, subSlotProcessingFrontLoadedDmrsEnabled, false, maxNPrbAlloc);
