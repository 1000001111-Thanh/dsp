...
void printPdschCellGrpDynPrms(const cuphyPdschCellGrpDynPrm_t& cellGrpDynPrms) {
    ...
    for (int ueGroupId = 0; ueGroupId < numUeGrps; ueGroupId++) {
        ...
        printf("  dmrsScrmId:          %4d\n", ueGrpPrms[ueGroupId].pDmrsDynPrm->dmrsScrmId);
...
void PdschParams::setDynPrms(const py::object& dynPrms) {
    ...
    m_pdschDynPrms.cuStream             = (cudaStream_t)cuStream;
    ...
    for (int ueGroupIdx = 0; ueGroupIdx < nUeGrps; ueGroupIdx++) {
        ...
        pDmrsDynPrm->dmrsScrmId = ueGrpPrms.attr("dmrsScrmId").cast<uint8_t>();
    }
    ...
    if (nPrecodingMatrices > 0) {
        for (int pmwIdx = 0; pmwIdx < nPrecodingMatrices; pmwIdx++) {
            ...
            m_cellGrpDynPrms.pPmwPrms[pmwIdx].nPorts  = pmwPrms.attr("nPorts").cast<uint8_t>();
            ...
            for (size_t idx = 0; idx < buf.size; idx++){
                m_cellGrpDynPrms.pPmwPrms[pmwIdx].matrix[idx].x = __float2half(ptr[idx].real());
                m_cellGrpDynPrms.pPmwPrms[pmwIdx].matrix[idx].y = __float2half(ptr[idx].imag());
            }
        }
    }
    ...
    for (int cellIdx = 0; cellIdx < nCells; cellIdx++) {
        py::array tempArray = tbInput[cellIdx];
        py::array_t<uint8_t, py::array::f_style | py::array::forcecast> tbInputArray = tempArray;
        py::buffer_info tbInputBuff = tbInputArray.request();
        m_tbInputPtr[cellIdx] = static_cast<uint8_t *>(tbInputBuff.ptr);
    }
    m_dataIn = {m_tbInputPtr.data(), cuphyPdschDataIn_t::CPU_BUFFER};
    ...
    if (!(std::string(py::str(dynPrms.attr("tbCRCDataIn"))) == "None")) {
        ...
        for (int cellIdx = 0; cellIdx < nCells; cellIdx ++) {
            py::array tempArray = tbCrcInput[cellIdx];
            py::array_t<uint8_t, py::array::f_style | py::array::forcecast> tbCrcInputArray = tempArray;
            py::buffer_info tbCrcInputBuff = tbCrcInputArray.request();
            tbCrcInputPtr[cellIdx] = static_cast<uint8_t *>(tbCrcInputBuff.ptr);
        }

        m_tbCrcDataIn               = {tbCrcInputPtr.data(), cuphyPdschDataIn_t::CPU_BUFFER};
        m_pdschDynPrms.pTbCRCDataIn = &m_tbCrcDataIn;

    }
    else {
        m_tbCrcDataIn               = {nullptr, cuphyPdschDataIn_t::CPU_BUFFER};
        m_pdschDynPrms.pTbCRCDataIn = &m_tbCrcDataIn;
    }
    ...
    for (int cellIdx = 0; cellIdx < nCells; cellIdx++) {
        ...
        if (type != CUPHY_C_16F && type != CUPHY_C_32F) {
