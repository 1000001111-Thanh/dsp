...
def save_pickle(data: Any, filename: str, s3: s3fs.S3FileSystem = None) -> None:
    ...
    Args:
        ...
        s3 (s3fs.S3FileSystem): The S3 filesystem to be used. Set to :code:`None` for local
            filesystem.
    """
    if s3 is not None:
        open_func = s3.open
...
def load_pickle(filename: str, s3: s3fs.S3FileSystem = None) -> Any:
