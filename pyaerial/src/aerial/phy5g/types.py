...
class CsiRsRrcDynPrms(NamedTuple):
    ...
    Args:
        ...
        prb_bitmap (List[int]): Bitmap defining frequency domain allocation. Counting is started
...
class PdschDataIn(NamedTuple):
    ...
    Args:
        tbInput (List[numpy.ndarray]): A list of transport block input buffers, one buffer per
            cell, indexed by `cellPrmDynIdx`. Each `tbInput` element points to a flat array
            with all TBs for that cell. Currently per-cell TB allocations are contiguous,
            zero-padded to byte boundary. Each element of the flat TB arrays is a numpy.uint8
            byte. When a cell is in test mode, then the buffer contains the PN23
