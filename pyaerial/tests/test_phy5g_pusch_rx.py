...
@pytest.mark.parametrize(
    ...
    ids=[int(test_case_number) for test_case_number in single_ue_grp_api_tc]
)
def test_pusch_rx(test_case_number):
    ...
    code_rates = [c / 10. for c in input_file["tb_pars"]["targetCodeRate"]]
...
@pytest.mark.parametrize(
    "test_case_number",
    pusch_config_api_tc,
    ids=[int(test_case_number) for test_case_number in pusch_config_api_tc]
)
def test_pusch_rx_config_api(pusch_config, test_case_number):
    """Run test cases on PUSCH receiver."""
    filename = glob.glob(TEST_VECTOR_DIR + f"TVnr_{test_case_number}_PUSCH_gNB_CUPHY_s*.h5")[0]
    try:
        input_file = h5.File(filename, "r")
...
def test_pusch_tx_to_rx(pdsch_tx, pusch_rx, test_case):
    ...
    for i in range(num_ues):
        tb_input_i = random_tb(
            qam[i], coderate[i], dmrs_position, 273, 0, 14, layers[i]
        )
